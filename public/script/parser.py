...
def addToSQL(data, cursor):
    ...
    last_forecast_update = datetime.strptime(
        f"{t[3]} {t[4]} {t[5]} {t[0]}{t[1]}", "%b %d, %Y %I:%M%p"
    )
                            ...
                            for node in paragraph.childNodes:
                                if node.nodeType == node.TEXT_NODE:
                                    if paragraph.getAttribute("class") == "period-name":
                                        ...
                                        paragraph.getAttribute("class") == "short-desc"
                                    ):
                                        tempD["short_description"] += (
                                            node.nodeValue + " "
                                        )
                                    elif paragraph.getAttribute("class") in [
                                        "temp temp-high",
                                        "temp temp-low",
                                    ]:
                                        tempD["temperature_text"] = node.nodeValue
                                        tempD["temperature"] = int(
                                            node.nodeValue.split()[1]
                                        )
                                elif node.nodeType == node.ELEMENT_NODE:
                                    if node.tagName == "img":
                                        tempD["long_description"] = node.getAttribute(
                                            "alt"
                                        )
                                else:
